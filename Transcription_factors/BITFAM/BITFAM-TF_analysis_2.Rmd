...
Res_tf_top100 <- importance(fit_rf)[order(importance(fit_rf)[, 1], decreasing = T), ][1:100]  ## Based on the feature importances identify the specific TFs for Resident cells
...
Z_norm <- normalize(Z)
Z_sum <- colSums(Z_norm)
top_TFs <- head(sort(Z_sum,decreasing = TRUE), n=100)
```
...
lables <- annotation%>%
  select("new_label")
Z_norm <- merge(Z_norm, lables, by = "row.names")
row.names(Z_norm) <- Z_norm$Row.names
Z_norm$Row.names <- NULL
```
