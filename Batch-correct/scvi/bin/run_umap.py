...
if __name__== "__main__":
    ...
    parser.add_argument('--pcs',
                        dest='pcs',
                        type=int,
                        help='Number of PC to consider to build neighbors graph.')
    args = parser.parse_args()

sc.settings.set_figure_params(dpi=200, frameon=False)
sc.set_figure_params(dpi=200)
sc.set_figure_params(figsize=(4, 4))
sc.settings.figdir = "./umap/figures/"
    
# read in data
adata = sc.read(args.input_object)

# compute knn and umap
sc.pp.neighbors(adata, n_neighbors=args.knn, n_pcs=args.pcs)
sc.tl.umap(adata)
sc.pl.umap(adata,
...
save_path = "./umap/csv/" + args.output_prefix + '_umap.csv'
