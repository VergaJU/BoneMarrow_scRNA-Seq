...
saveRDS(pred, file = str_replace(dat, ".Rds", "_singleR_results.Rds"))
saveRDS(dat, file = str_replace(dat, ".Rds", "_labelled.Rds"))
write.table(df, file =str_replace(dat, ".Rds", '_cell_labels.csv'), sep = '\t', row.names = T, col.names = T, quote = F)