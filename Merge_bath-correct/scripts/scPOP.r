...
tbl <- list.files() %>% map_df(~read.csv(., sep = '\t')) 
tbl <- tbl %>% select(-lisi_label.1, -silWidth_label.1)
...
tbl$sumZscore <-  calc_sumZscore(tbl, 'batch')
...
sc_mtx <- get_scaled_mtx(tbl %>% select(-sumZscore), 'batch') 
...
ggplot(sc_tbl, aes(x = metric, y = run, fill = as.numeric(value))) +
  geom_tile() +
  scale_x_discrete(limits = c('ari_label', 'nmi_label', 'lisi_batch', 'lisi_label', 'silWidth_batch', 'silWidth_label', 'sumZscore')) +
...
ggsave('scPOP_selected_results.png', height = 12, width = 7)

write.table(tbl, 'scPOP_selected_results.csv', sep='\t', row.names = F, quote = F)